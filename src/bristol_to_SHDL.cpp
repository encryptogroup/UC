...
uint64_t read_Bristol_circuit(string string_file, uint32_t& input_num_total, uint32_t& output_num){
    string out_file_name = string_file + "_SHDL.circuit";
   ...
   for(uint32_t i = input_num_total; i < gate_num + input_num_total; ++i){
        ...
        if(current->type == 0){
            if((left_p->type == 2 && right_p->type == 2) || left_p->type != 2 && right_p->type != 2){  //either a XOR b    or    NOT a XOR NOT b
                file2 << "0 1 1 0";
            }
...
int main(void){
    ...
    const char* filename[3] = {
        ...
        "../circuits/comparator_32bit_signed_lt.txt",
